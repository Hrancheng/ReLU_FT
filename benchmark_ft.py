...
# for mode in ['bwd']:
    for headdim in [256]:
    # for headdim in [128]:
        for seqlen in [512, 1024, 2048, 4096, 8192, 16384, 32768]:
        # for seqlen in [8192]:
            nheads = dim // headdim
            # nheads = 24
            # headdim = 64
            ...
            # for causal in [True]:
                ...
                # Write the header if the file is empty
                        if file_is_empty:
                            writer.writerow(["MeanTime_ms", "TFLOPS"])                    # Write the data row
                        mean_time_ms = m0.mean * 1e3
                        tflops = (f / m0.mean) * 1e-12
                        writer.writerow([f"{mean_time_ms:.3f}", f"{tflops:.1f}"])
                if headdim <= 128:
